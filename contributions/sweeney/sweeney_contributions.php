...
mysql_select_db("wcfcourier_com");
	...
	$list = mysql_query("SELECT DISTINCT State FROM house50_sweeney_contributions ORDER BY Name_of_Contributor");
	$list_dollar = mysql_query("SELECT DISTINCT Contribution_Amount FROM house50_sweeney_contributions ORDER BY Contribution_Amount");
	...
	if ($_GET["searchWord"] || $_GET["select"] || $_GET["select_dollar"] || $_GET["select_sort"]) {
		...
		if ($select == 'all') {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND Contribution_Amount>='$select_dollar' ORDER BY Name_of_Contributor");
		} else {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND State='$select' AND Contribution_Amount>='$select_dollar' ORDER BY Name_of_Contributor");
		}
		...
		if ($select == 'all' && $select_sort == 'Contribution_Amount') {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND Contribution_Amount>='$select_dollar' ORDER BY $select_sort DESC LIMIT $set_limit, $limit");
		} else if ($select == 'all') {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND Contribution_Amount>='$select_dollar' ORDER BY $select_sort LIMIT $set_limit, $limit");
		} else if ($select_sort == 'Contribution_Amount') {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND State='$select' AND Contribution_Amount>='$select_dollar' ORDER BY $select_sort DESC LIMIT $set_limit, $limit");
		} else {
			$q = mysql_query("SELECT * FROM  house50_sweeney_contributions WHERE Name_of_Contributor LIKE '%$word%' AND State='$select' AND Contribution_Amount>='$select_dollar' ORDER BY $select_sort LIMIT $set_limit, $limit");
		}
		
	} else {
		$q = mysql_query("SELECT * FROM house50_sweeney_contributions ORDER BY Contribution_Date");
		$URL = "";
		...
		$q = mysql_query("SELECT * FROM house50_sweeney_contributions ORDER BY Contribution_Date LIMIT $set_limit, $limit");
	}
	...
	echo("<div class='row'>
				<div class='span12'>
					...
					<h2>Contributions to Sweeney for State House</h2>
				</div>
			</div>
			...
			<div class='row'>
				<div class='span8'>
					...
					<td>
					<p>Representative Annette Sweeney of Alden raised a total of $60,071 between April 2011 and May 2012, according to her latest filing to the Iowa Ethics & Campaign Disclosure Board.</p>
					<div class='hidden-phone'>
					...
					<p>Sweeney relied heavily on small donations with just 18 of her 278 donations valued at $1000 or more. Her largest donation came from Steve and Glenda Howard of Iowa Falls, which was valued at $5000.</p>
					<div class='hidden-tablet'>
					<p>Sweeney, unlike her opponent Representative Pat Grassley, relied almost exclusively on in-state contributions with almost 95 percent of her donors residing in Iowa. About 50 percent of Grassley's contributions came from Iowa residents. A total of 14 donations for Sweeney did not list a state.</p>
				...
				<div class='visible-phone'>
					<ul>
					<li><h3>
					<a href='http://wcfcourier.com/app/special/grassley_sweeney/contributions/grassley/'>Compare with Representative Pat Grassley</a>
					</h3></l>
					</ul>
				</div>
				<div class='hidden-phone'>
	...
	if($prev_page >= 1) {
		echo("<li><a class='moreListings' href=/app/special/grassley_sweeney/index.php?$URL&limit=$limit&page=$prev_page>&laquo;Previous</a></li>");
	}
	...
	for($a = 1; $a <= $total_pages; $a++){
		if($a == $page) {
			...
			echo(" <li><a class='moreListings' href=/app/special/grassley_sweeney/index.php?$URL&limit=$limit&page=$a>$a</a></li>");
		}
	}
	...
	if($next_page <= $total_pages) {
		echo("<li><a class='moreListings' href=/app/special/grassley_sweeney/index.php?$URL&limit=$limit&page=$next_page>Next&raquo;</a></li>");
	}
	echo "</ul></div>";
	//all done
mysql_close();
