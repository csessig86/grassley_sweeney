...
mysql_select_db("wcfcourier_com");
	...
	$list = mysql_query("SELECT DISTINCT City FROM house50_grassley_expenditures ORDER BY Expenditure");
	$list_dollar = mysql_query("SELECT DISTINCT Expenditure_Amount FROM house50_grassley_expenditures ORDER BY Expenditure_Amount");
	...
	if ($_GET["searchWord"] || $_GET["select"] || $_GET["select_dollar"] || $_GET["select_sort"]) {
		...
		if ($select == 'all') {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND Expenditure_Amount>='$select_dollar' ORDER BY Expenditure");
		} else {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND City='$select' AND Expenditure_Amount>='$select_dollar' ORDER BY Expenditure");
		}
		...
		if ($select == 'all' && $select_sort == 'Expenditure_Amount') {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND Expenditure_Amount>='$select_dollar' ORDER BY $select_sort DESC LIMIT $set_limit, $limit");
		} else if ($select == 'all') {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND Expenditure_Amount>='$select_dollar' ORDER BY $select_sort LIMIT $set_limit, $limit");
		} else if ($select_sort == 'Expenditure_Amount') {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND City='$select' AND Expenditure_Amount>='$select_dollar' ORDER BY $select_sort DESC LIMIT $set_limit, $limit");
		} else {
			$q = mysql_query("SELECT * FROM  house50_grassley_expenditures WHERE Expenditure LIKE '%$word%' AND City='$select' AND Expenditure_Amount>='$select_dollar' ORDER BY $select_sort LIMIT $set_limit, $limit");
		}
		
	} else {
		$q = mysql_query("SELECT * FROM house50_grassley_expenditures ORDER BY Expenditure_Date");
		$URL = "";
		...
		$q = mysql_query("SELECT * FROM house50_grassley_expenditures ORDER BY Expenditure_Date LIMIT $set_limit, $limit");
	}
	
	//query: **EDIT TO YOUR TABLE NAME, ECT. 
	echo("<div class='row'>
				<div class='span12'>
					<h2>Expenditures by Citizens for Pat Grassley</h2>
				</div>
			</div>
			...
			<div class='row'>
				<div class='span8'>
					...
					<td>
					<p>Representative Pat Grassley of New Hartford spent a total of $69,867 between last year and May 2012, according to his latest filing to the Iowa Ethics & Campaign Disclosure Board.</p>
					<div class='hidden-phone'>
					...
					<p>The finance disclosures show Grassley's campaign made 37 expenditures, compared to 122 expenditures by his opponent's campaign. But Grassley outspent his opponent, with Representative Annette Sweeney spending about $48,563.</p>
					<div class='hidden-tablet'>
					<p>Almost all expenditures by both candidates were made to people and businesses within Iowa. These included expenditures for newspaper advertising, campaign merchandise and postage for campaign items. Grassley's largest expenditure was to Victory Enterprises in Davenport, Iowa for $3,575.</p>
				...
				<div class='visible-phone'>
					<ul>
					<li><h3>
					<a href='http://wcfcourier.com/app/special/grassley_sweeney/expenditures/sweeney/'>Compare with Representative Annette Sweeney</a>
					</h3></l>
					</ul>
				</div>
				<div class='hidden-phone'>
					<div class='btn-group'>
						...
						<ul class='dropdown-menu'>
            				<li><a href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=15&page=1>15</a></li>
							<li><a href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=25&page=1>25</a></li>
							<li><a href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=50&page=1>50</a></li>
          				</ul>
					</div>
				</div>
			</div>
	...
	if($prev_page >= 1) {
		echo("<li><a class='moreListings' href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=$limit&page=$prev_page>&laquo;Previous</a></li>");
	}
	...
	for($a = 1; $a <= $total_pages; $a++){
		if($a == $page) {
			...
			echo(" <li><a class='moreListings' href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=$limit&page=$a>$a</a></li>");
		}
	}
	...
	if($next_page <= $total_pages) {
		echo("<li><a class='moreListings' href=/app/special/grassley_sweeney/expenditures/grassley/index.php?$URL&limit=$limit&page=$next_page>Next&raquo;</a></li>");
	}
	echo "</ul></div>";
	//all done
mysql_close();
